...
# of the rack audio threads and assign them to certain cpu cores
#
# this is done this way as when the rack internal realtime
# priority setting is used the system sometimes locked up on startup
#
# the cpu affinity is used to push the rack audio threads away
...
if [ "${DESIRED_CPU_AFFINITY}" = "" ]; then
  # on which cpu cores should rack run - default: 2,3 (0 = cpu1 etc. - i.e. 3+4)
...
RACK_PID=`pidof Rack`
if [ "${RACK_PID}" != "" ]; then
  for i in `top -p ${RACK_PID} -b -n 1 -H | sed 1,7d | grep Engine | awk '{print $1}'`; do
