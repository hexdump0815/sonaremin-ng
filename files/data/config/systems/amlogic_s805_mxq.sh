grep -q 'TRONFY MXQ S805$' /proc/device-tree/model
if [ "$?" = "0" ]; then
  # mxq
  ...
  echo "SYSTEM_MODEL=s805" > /data/config/info.txt
  echo "SYSTEM_MODEL_DETAILED=amlogic_s805_mxq" >> /data/config/info.txt
