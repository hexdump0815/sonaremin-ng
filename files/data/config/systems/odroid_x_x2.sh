grep -q 'Hardkernel ODROID-X2* board based on Exynos4412$' /proc/device-tree/model
if [ "$?" = "0" ]; then
  # odroid x
  ...
  if [ -f /data/config/custom/audio-setup.sh ]; then
    ...
    cp /data/config/qjackctl/QjackCtl.conf-odroid_x /data/config/qjackctl/QjackCtl.conf
  ...
  echo "SYSTEM_MODEL_DETAILED=odroid_x" >> /data/config/info.txt
