grep -q 'Eachlink H6 Mini$' /proc/device-tree/model
if [ "$?" = "0" ]; then
  # allwinner h6 tv box
  ...
  echo "SYSTEM_MODEL=h6" > /data/config/info.txt
  echo "SYSTEM_MODEL_DETAILED=allwinner_h6_eachlink_tv_box" >> /data/config/info.txt
