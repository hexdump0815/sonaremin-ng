...
mount -o bind /dev ${S_MOUNT_POINT}/dev
mount -o bind /dev/pts ${S_MOUNT_POINT}/dev/pts
mount -t sysfs /sys ${S_MOUNT_POINT}/sys
mount -t proc /proc ${S_MOUNT_POINT}/proc

# finalize script which is run chrooted per system
if [ -x ${IMAGEBUILDER}/systems/${1}/finalize-chroot.sh ]; then
  cp ${IMAGEBUILDER}/systems/${1}/finalize-chroot.sh ${S_MOUNT_POINT}/finalize-chroot.sh
  chroot ${S_MOUNT_POINT} /finalize-chroot.sh ${1} ${2} focal
  rm -f ${S_MOUNT_POINT}/finalize-chroot.sh
fi

# umount all the extra stuff mounted for chroot usage as we are done with chroots now
umount ${S_MOUNT_POINT}/proc ${S_MOUNT_POINT}/sys ${S_MOUNT_POINT}/dev/pts ${S_MOUNT_POINT}/dev
